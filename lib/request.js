...
function getContent(url, callback) {
  ...
  phantom = childProcess.execFile(
    ...
    [
      ...
      '--ssl-protocol=any',
      '--load-images=' + config.loadImages,
      __dirname + '/phantom-server.js',
      url
    ],
    {
      maxBuffer: config.phantomMaxBuffer,
...
function next() {
  ...
  if (nextItem !== undefined) {
    logger.info('Picking up request from queue.', nextItem.url);
    getContent(nextItem.url, nextItem.callback);
  }
}

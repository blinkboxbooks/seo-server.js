...
var getContent = function(url, callback) {
  ...
  var phantom = childProcess.execFile('phantomjs',
    ...
    {timeout: config.phantomTimeout, killSignal: 'SIGKILL'}, function(error, stdout) {
      ...
      if (error !== null && error.killed) {
        log.info('Error from inside the callback of the childProcess.exec:', error)
        callback(504);
      } else {
        callback(stdout);
      }
  ...
  phantom.stdout.on('data', function(data){
    log.info('STDOUT from Phantom ', data);
  });

  phantom.stderr.on('data', function (data) {
      log.info('STDERR from Phantom: ', data);
  ...
  phantom.on('exit', function(code) {
    ...
    log.info('Phantom JS finished in '+((timeFinish-timeStart)/1000)+' seconds, exit code ' + code);
  });
...
respond = function (req, res) {
  ...
  log.info('Incoming request, headers:', req.headers);
  ...
  sendURL({
    ...
    callback: function (content) {
      log.info('Return to callback', content);

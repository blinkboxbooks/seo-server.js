...
function removeScriptTags(content) {
  // Remove all script tags other than those of type "application/ld+json":
  return content && content.replace(/<script(?!\s+type="application\/ld\+json")(?:.*?)>(?:[\S\s]*?)<\/script>/gi, '');
}
...
checkComplete = function () {
  ...
  if (pageLoaded &&
    ...
    var output = config.removeScriptTags ? removeScriptTags(page.content) : page.content;
